// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'syllabus_model.dart';

// **************************************************************************
// JsonSerializableGenerator
// **************************************************************************

SyllabusModel _$SyllabusModelFromJson(Map<String, dynamic> json) =>
    SyllabusModel(
      status: json['status'] as bool?,
      message: json['message'] as String?,
      records: (json['records'] as List<dynamic>?)
          ?.map((e) => SyllabusRecord.fromJson(e as Map<String, dynamic>))
          .toList(),
      commondata: json['commondata'] as String?,
      formStatus: json['formStatus'] as String?,
      sub_type: json['sub_type'] as String?,
      grades: json['grades'] as String?,
      subject_mark: json['subject_mark'] as String?,
    );

Map<String, dynamic> _$SyllabusModelToJson(SyllabusModel instance) =>
    <String, dynamic>{
      'status': instance.status,
      'message': instance.message,
      'records': instance.records,
      'commondata': instance.commondata,
      'formStatus': instance.formStatus,
      'sub_type': instance.sub_type,
      'grades': instance.grades,
      'subject_mark': instance.subject_mark,
    };

SyllabusRecord _$SyllabusRecordFromJson(Map<String, dynamic> json) =>
    SyllabusRecord(
      syllabusID: json['syllabusID'] as String?,
      title: json['title'] as String?,
      filePath: json['filePath'] as String?,
      classID: json['classID'] as String?,
      className: json['className'] as String?,
      uploadDate: json['uploadDate'] as String?,
      empID: json['empID'] as String?,
      branchID: json['branchID'] as String?,
      school_code: json['school_code'] as String?,
      type: json['type'] as String?,
    );

Map<String, dynamic> _$SyllabusRecordToJson(SyllabusRecord instance) =>
    <String, dynamic>{
      'syllabusID': instance.syllabusID,
      'title': instance.title,
      'filePath': instance.filePath,
      'classID': instance.classID,
      'className': instance.className,
      'uploadDate': instance.uploadDate,
      'empID': instance.empID,
      'branchID': instance.branchID,
      'school_code': instance.school_code,
      'type': instance.type,
    };
